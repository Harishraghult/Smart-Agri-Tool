...
def download_kaggle(slug: str, output_dir: Path, name: str) -> bool:
    ...
    try:
        ...
        print(f"  [FAIL] Kaggle download failed for {name}: {e.stderr.strip()}")
        print(f"    Manual download: https://www.kaggle.com/datasets/{slug}")
        return False
    except FileNotFoundError:
        print(f"  [FAIL] Kaggle CLI not found. Install: pip install kaggle")
        print(f"    Manual download: https://www.kaggle.com/datasets/{slug}")
        return False
...
def clone_github(url: str, output_dir: Path, name: str) -> bool:
    ...
    if target.exists() and any(target.iterdir()):
        print(f"  [SKIP] {name} already exists, skipping clone")
        return True
    try:
        subprocess.run(
        ...
        print(f"  [FAIL] Git clone failed for {name}: {e.stderr.strip()}")
        print(f"    Manual clone: git clone {url}")
        return False
    except FileNotFoundError:
        print(f"  [FAIL] Git not found. Install git, then retry.")
        print(f"    Manual clone: git clone {url}")
        return False
...
def main():
    ...
    for ds in KAGGLE_DATASETS:
        ...
        if args.skip_existing and target.exists() and any(target.iterdir()):
            print(f"  [SKIP] {ds['name']} already exists, skipping")
